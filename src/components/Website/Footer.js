...
const Footer = forwardRef(function Footer(props, ref) {
  ...
  const socialLinks = [
    {
      ...
      icon: (
        <svg width="20" height="14" viewBox="0 0 20 14" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      icon: (
        <svg width="14" height="14" viewBox="0 0 14 14" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      icon: (
        <svg width="21" height="20" viewBox="0 0 21 20" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <footer ref={ref} id="site-footer" className="w-full px-[5vw] py-12">
      <div className="bg-gradient-to-br from-[#1a1a2e] to-[#16213e] rounded-[2rem] px-8 lg:px-12 py-12 text-white">
        {/* Main Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="inline-block mb-6">
              <img src={PLogo} alt="Hidayah Logo" className="h-10 brightness-0 invert" />
            </Link>
            <p className="text-gray-300 text-lg mb-6 max-w-md leading-relaxed">
              First AI-Powered Islamic Design & Tech Hub. Crafting meaningful digital experiences for the global Muslim community.
            </p>

            {/* Newsletter */}
            <div className="mb-6">
              <h4 className="text-white font-semibold mb-4 flex items-center gap-2">
                <Mail className="w-5 h-5 text-br-color" />
              ...
              <form onSubmit={handleSubmit} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white/10 border border-white/20 rounded-full px-5 py-3 text-white placeholder:text-gray-400 focus:outline-none focus:border-br-color transition"
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="cursor-cta bg-br-color hover:bg-teal-600 text-white px-6 py-3 rounded-full font-semibold flex items-center gap-2 transition-all duration-300 hover:shadow-lg hover:shadow-br-color/30 disabled:opacity-60"
                >
            ...
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="cursor-cta w-12 h-12 bg-white/10 hover:bg-br-color text-white rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
                >
                  {social.icon}
                </a>
              ))}
              ...
              {companyLinks.map((link) => (
                <li key={link.to}>
                  <NavLink
                    to={link.to}
                    className={({ isActive }) =>
                      `text-gray-300 hover:text-br-color transition-colors ${isActive ? "text-br-color font-medium" : ""}`
                    }
                  >
                    {link.name}
                  </NavLink>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {servicesLinks.map((service) => (
                <li key={service}>
                  <Link to="/contact" className="text-gray-300 hover:text-br-color transition-colors">
                    {service}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-white/10 mt-10 pt-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-gray-400 text-sm">
              © {new Date().getFullYear()} Hidayah. All rights reserved.
            </p>

            <div className="flex items-center gap-6 text-sm">
              <NavLink to="/privacy-policy" className="text-gray-400 hover:text-br-color transition-colors">
                Privacy Policy
              </NavLink>
              <NavLink to="/cookies" className="text-gray-400 hover:text-br-color transition-colors">
                Cookies
              </NavLink>
            </div>
            ...
            <button
              ...
              className="cursor-cta w-11 h-11 bg-br-color hover:bg-teal-600 text-white rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
